...
variable "sql" {
  ...
  validation {
    condition     = var.sql.create == false || (var.sql.create == true && (var.sql.type == "server" || (var.sql.type == "managed" && var.sql.sku_name != "ElasticPool")))
    error_message = "Managed instances can not be ElasticPool sku."
  }
  validation {
    condition     = var.sql.create == false || (var.sql.create == true && contains(keys(var.sql.tags), "tagname") != true)
    error_message = "The key `tagname` is just an example. Please remove it from the additional tags."
  }
  validation {
    condition     = var.sql.create == false || (var.sql.create == true && contains(keys(var.rg_config), var.sql.rg_key))
    error_message = "Resource group key name must be defined as in rg_config variable"
  }
  validation {
    condition     = var.sql.create == false || (var.sql.create == true && contains(keys(var.vnet_config.subnets), var.sql.subnet))
    error_message = "Subnet key name must be defined as in vnet_config.subnets variable"
  }
  validation {
    condition = var.sql.create == false || (var.sql.create == true && (var.sql.type == "managed" || (var.sql.type == "server" && contains([
...
variable "terraform_version" {
  ...
  validation {
    ...
    condition     = can(regex("^(?:[0-9]|[1-9][0-9])\\.(?:[0-9]|[1-9][0-9])\\.(?:[0-9]|[1-9][0-9])$", var.terraform_version))
  }
}
...
variable "vm_ux" {
  ...
  validation {
    condition     = var.vm_ux.hostname == "" || (var.vm_ux.hostname != "" && contains(keys(var.rg_config), var.vm_ux.rg_key))
    error_message = "Resource group key name must be defined as in rg_config variable"
  }
  validation {
    condition     = var.vm_ux.hostname == "" || (var.vm_ux.hostname != "" && contains(keys(var.vnet_config.subnets), var.vm_ux.subnet))
    error_message = "Subnet key name must be defined as in vnet_config.subnets variable"
  }
}
...
variable "vm_win" {
  ...
  validation {
    condition     = var.vm_win.hostname == "" || (var.vm_win.hostname != "" && contains(keys(var.rg_config), var.vm_win.rg_key))
    error_message = "Resource group key name must be defined as in rg_config variable"
  }
  validation {
    condition     = var.vm_win.hostname == "" || (var.vm_win.hostname != "" && contains(keys(var.vnet_config.subnets), var.vm_win.subnet))
    error_message = "Subnet key name must be defined as in vnet_config.subnets variable"
  }
}
...
variable "module_version_tags" {
  ...
  validation {
    condition     = can(regex("^(?:[0-9]|[1-9][0-9])\\.(?:[0-9]|[1-9][0-9])\\.(?:[0-9]|[1-9][0-9])$", var.module_version_tags))
